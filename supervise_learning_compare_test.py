...
def train(epoch, train_loader, learner, args):
    ...
    train_acc = confusion.acc()
    return loss_record, train_acc
...
def run(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        loss_temp, train_acc = train(epoch, train_loader, learner, args)
    ...
    test_acc = KPI
    return KPI, run_loss, train_acc

def get_args(model_type, model_name, loss, dataset, epoch, lr,schedule,optimizer):
    ...
    parser.add_argument('--model_name', type=str, default=model_name, help="LeNet(default)|LeNetC|VGGS|VGG8|VGG16|ResNet18|ResNet101 ...")
    parser.add_argument('--dataset_type', type=str, default='default')
    parser.add_argument('--dataset', type=str, default=dataset, help="MNIST(default)|CIFAR10|CIFAR100|Omniglot|Omniglot_eval_Old_Church_Slavonic ...")
    ...
    parser.add_argument('--epochs', type=int, default=epoch, help="End epoch")
    parser.add_argument('--batch_size', type=int, default=100)
    parser.add_argument('--lr', type=float, default=lr, help="Learning rate 0.001")
    parser.add_argument('--loss', type=str, default=loss, choices=['CE', 'KCL', 'MCL', 'DPS'],
...
def test(model_type,model, dataset, epoch,lr,schedule,optimizer):
    start_time = time.time()
    a, loss_ce, train_acc_ce = run(get_args( model_type, model, "CE", dataset, epoch,lr,schedule,optimizer))
    end_time = time.time()
    CE_time = end_time - start_time
    start_time = time.time()
    a, loss_kcl, train_acc_kcl = run(get_args(model_type, model, "KCL", dataset, epoch,lr,schedule,optimizer))
    end_time = time.time()
    KCL_time = end_time - start_time
    start_time = time.time()
    a, loss_mcl, train_acc_mcl = run(get_args(model_type, model, "MCL", dataset, epoch,lr,schedule,optimizer))
    end_time = time.time()
    MCL_time = end_time - start_time
    print(CE_time,KCL_time,MCL_time)
...
if __name__ == '__main__':
    ...
    schedule = [60,80]
    optimizer = 'Adam'
    test(model_type, model, dataset, epoch, lr, schedule, optimizer)
