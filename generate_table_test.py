...
from demo import prepare_task_target
...
def train(epoch, train_loader, learner, args):
    ...
    if args.loss == 'CE':
        ...
        args.cluster2Class = confusion.optimal_assignment(
            train_loader.num_classes)  # Save the mapping in args to use in eval
...
def run(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        loss_temp = train(epoch, train_loader, learner, args)
        run_loss.append(loss_temp)
...
def get_args(argv, model_type, model_name, lo, dataset, batch_size, schedule, epochs, optimizer='Adam'):
    # This function prepares the variables shared across demo.py

    parser = argparse.ArgumentParser()
    parser.add_argument('--gpuid', nargs="+", type=int, default=[0, 1, 2],
                        help="The list of gpuid, ex:--gpuid 3 1. Negative value means cpu-only")
    parser.add_argument('--model_type', type=str, default=model_type, help="lenet(default)|vgg|resnet")
    parser.add_argument('--model_name', type=str, default=model_name,
                        help="LeNet(default)|LeNetC|VGGS|VGG8|VGG16|ResNet18|ResNet101 ...")
    parser.add_argument('--dataset_type', type=str, default='default')
    parser.add_argument('--dataset', type=str, default=dataset,
                        help="MNIST(default)|CIFAR10|CIFAR100|Omniglot|Omniglot_eval_Old_Church_Slavonic ...")
    ...
    parser.add_argument('--epochs', type=int, default=epochs, help="End epoch")
    parser.add_argument('--batch_size', type=int, default=batch_size)
    parser.add_argument('--lr', type=float, default=0.001, help="Learning rate")
    parser.add_argument('--loss', type=str, default=lo, choices=['CE', 'KCL', 'MCL', 'DPS'],
                        help="CE(cross-entropy)|KCL|MCL(default)|DPS(Dense-Pair Similarity)")
    parser.add_argument('--schedule', nargs="+", type=int, default=schedule,
    ...
    parser.add_argument('--SPN_model_name', type=str, default='VGGS',
                        help="This option is only valid when use_SPN=True")
    parser.add_argument('--SPN_pretrained_model', type=str, default='outputs/Omniglot_VGGS_DPS.model.pth',
                        help="This option is only valid when use_SPN=True")
...
if __name__ == '__main__':
    ...
    for i, dataset in enumerate(datasets):
        if dataset == "MNIST":
            ...
            epochs = 1 #30
            acc_ce, loss_ce = run(
                get_args(sys.argv[1:], "lenet", "LeNet", "CE", dataset, batch_size, drop_epoch, epochs, optimiser))
            acc_kcl, loss_kcl = run(
                get_args(sys.argv[1:], "lenet", "LeNet", "KCL", dataset, batch_size, drop_epoch, epochs, optimiser))
            acc_mcl, loss_mcl = run(
                get_args(sys.argv[1:], "lenet", "LeNet", "MCL", dataset, batch_size, drop_epoch, epochs, optimiser))
            print('dataset:', dataset)
            print('mcl', acc_mcl, loss_mcl)
            print('kcl', acc_kcl, acc_kcl)
            print('ce', acc_ce, loss_ce)
        elif dataset == "CIFAR10":
            networks = ["LeNet", "VGG8", "VGG16", "ResNet18", "ResNet34", "ResNet50", "ResNet101"] #
            for j, network in enumerate(networks):
                ...
                if network[0] == 'L':
                    ...
                    assert ('wrong model_type')
                acc_ce, loss_ce = run(
                    get_args(sys.argv[1:], model_type, network, "CE", dataset, batch_size, drop_epoch, epochs,
                             optimiser))
                acc_kcl, loss_kcl = run(
                    get_args(sys.argv[1:], model_type, network, "KCL", dataset, batch_size, drop_epoch, epochs,
                             optimiser))
                acc_mcl, loss_mcl = run(
                    get_args(sys.argv[1:], model_type, network, "MCL", dataset, batch_size, drop_epoch, epochs,
                             optimiser))
                print('dataset:', dataset)
            ...
            drop_epoch = [100, 150]
            epochs = 1 #180
            acc_mcl, loss_mcl = run(get_args(sys.argv[1:], "vgg", "VGG8", "MCL", dataset, batch_size, drop_epoch, epochs))
            acc_kcl, loss_kcl = run(get_args(sys.argv[1:], "vgg", "VGG8", "KCL", dataset, batch_size, drop_epoch, epochs))
            acc_ce, loss_ce = run(get_args(sys.argv[1:], "vgg", "VGG8", "CE", dataset, batch_size, drop_epoch, epochs))
            print('dataset:', dataset)
            print('mcl', acc_mcl, loss_mcl)
            print('kcl', acc_kcl, acc_kcl)
            print('ce', acc_ce, loss_ce)
